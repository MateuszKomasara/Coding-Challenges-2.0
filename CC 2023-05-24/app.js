...
function span(arr, predicate) {
    if (arr.every(x => predicate(x))) {
     ...
     const failedIndex = arr.findIndex((element) => !predicate(element));
     const passedSlice = arr.slice(0, failedIndex);
     const failedSlice = arr.slice(failedIndex);
        return [passedSlice, failedSlice];
   }
